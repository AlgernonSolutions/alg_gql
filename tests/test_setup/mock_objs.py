...
def dev_dynamo():
    patch_obj = patch('toll_booth.obj.index.index_manager.boto3.resource')
...
def prod_s3_stored_data():
    patch_obj = patch('toll_booth.tasks.aws_tasks.boto3.resource')
    mock_resource = patch_obj.start()
    mock_resource.return_value = boto3.Session(profile_name='prod').resource('s3')
    return mock_resource, patch_obj
