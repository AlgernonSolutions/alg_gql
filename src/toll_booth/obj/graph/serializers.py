...
class GqlDecoder(json.JSONDecoder):
    ...
    @staticmethod
    def object_hook(obj):
        logging.debug(f'starting a GQL Decoding operation for: {obj}')
        if '_alg_class' not in obj:
            logging.debug(f'object is not an algernon object, let the default handle it')
            return obj
        alg_class = obj['_alg_class']
        obj_value = obj['value']
        logging.debug(f'object is an algernon object, class: {alg_class}, value: {obj_value}')
        ...
        if '_alg_module' not in obj:
            logging.debug(f'_alg_module not found in {obj_value} for {alg_class}, possibly error brewing')
            return obj
        alg_module = obj['_alg_module']
        logging.debug(f'alg_module for {obj_value} for {alg_class} is {alg_module}')
        ...
        logging.debug(f'after remake, {alg_module}.{alg_class} is {alg_obj}')
        alg_gql = alg_obj.to_gql
        logging.debug(f'gql variant of {alg_obj} is {alg_gql}')
        return alg_gql
